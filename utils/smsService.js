...
/**
 * Send a specific (server-generated) OTP to a phone number via 2Factor.in.
 * Used when the same code must also be deliverable by another channel (email),
 * so the code is generated by us instead of AUTOGEN.
 * @param {string} phone - Phone number (e.g., 91XXXXXXXXXX)
 * @param {string|number} otp - The OTP code to send
 * @returns {string} - 2Factor details string
 */
const sendCustomOTP = async (phone, otp) => {
  const response = await axios.get(
    `${BASE_URL}/${TWO_FACTOR_API_KEY}/SMS/${phone}/${otp}`
  );
...
module.exports = { sendOTP, sendCustomOTP, verifyOTP };
