...
exports.claimLevelReward = async (req, res) => {
  try {
    ...
    // Calculate reward and payment split. Stored on the Pending tx so admin
    // accept uses the exact numbers computed at request time.
    ...
    // STEP 2: Create Pending Transaction. The levels-array entry stays in
    // "Processing" until admin acts. Admin Accept will send crypto and flip it
    // to "Claimed"; Admin Reject will $pull the entry so the user can retry.
    const label = hasDivisions ? `Level ${level} Part ${division}` : `Level ${level}`;
