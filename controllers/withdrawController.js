 ...
 * User-side: atomically lock package (Matured → PendingWithdraw) and create a
 * Pending Transaction with full distribution metadata. No crypto, no retopup
 * credit, no CGT Homes call here. Admin Accept performs all three; Admin
 * Reject restores Matured.
 */
const withdrawHybrid = async (req, res) => {
  try {
    ...
    // Wallet-address format validation is enforced on the admin-accept side
    // before the actual USDT send.

    // ATOMIC LOCK: flip Matured → PendingWithdraw in a single op. The query
    // filters by owner + type + status so a race or double-click cannot lock
    ...
    if (!lockedPackage) {
      // Diagnose why the lock failed so the user sees a useful error.
    ...
    // Pending Transaction — admin Accept reads metadata to drive the actual
    // distribution promised at request time.
...
const withdrawHybridBalance = async (req, res) => {
  try {
    ...
    // Block if user already has an unresolved hybrid-balance request.
    const pendingWithdrawal = await Transaction.findOne({
      userId,
    ...
    // ATOMIC: lock balance only if sufficient (prevents double-spend race).
